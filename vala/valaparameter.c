...
ValaParameter* vala_parameter_copy (ValaParameter* self) {
	...
	if (!_tmp0_) {
		...
		_tmp15_ = _result_;
		_tmp16_ = ((ValaCodeNode*) self)->attributes;
		_tmp17_ = g_list_copy (_tmp16_);
		__g_list_free__vala_code_node_unref0_0 (((ValaCodeNode*) _tmp15_)->attributes);
		((ValaCodeNode*) _tmp15_)->attributes = _tmp17_;
		result = _result_;
		return result;
	} else {
		ValaParameter* _tmp18_;
		_tmp18_ = vala_parameter_new_with_ellipsis (NULL);
		result = _tmp18_;
		return result;
	}
}
...
static gboolean vala_parameter_real_check (ValaCodeNode* base, ValaCodeContext* context) {
	...
	if (!_tmp39_) {
		...
		if (_tmp44_) {
			...
			_tmp46_ = _tmp45_;
			_tmp43_ = !VALA_IS_ARRAY_TYPE (_tmp46_);
		} else {
			_tmp43_ = FALSE;
		}
		_tmp47_ = _tmp43_;
	...
	if (_tmp62_ != NULL) {
		...
		_tmp66_ = _tmp65_;
		if (VALA_IS_NULL_LITERAL (_tmp66_)) {
		...
		if (_tmp73_) {
			...
			_tmp84_ = _tmp83_;
			if (!VALA_IS_NULL_LITERAL (_tmp84_)) {
